...
class ScreenShotsWin(QMainWindow):
    ...
    @staticmethod
    def screenshotsFullScreen():
        des = QApplication.desktop()
        screen = QApplication.primaryScreen()
        if screen:
            pix = screen.grabWindow(des.winId())
            pix.save('./save/1.jpg')
        return pix
    ...
    def screenshots(self, start, end):
        ...
        if screen:
            ...
            self.pix = screen.grabWindow(des.winId(), x, y, width, height)
            # if pix:
            #     self.scene.addItem(GraphicsPixmapItem(pix))
